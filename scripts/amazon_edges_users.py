# Version 2: edited to use the 2018 version of the dataset at https://nijianmo.github.io/amazon/index.html 
import json
import gzip
import pandas as pd
import numpy as np
...
def open_csv(filename: str) -> pd.DataFrame:
    with open(filename) as file:
        #original_df = pd.read_csv(file, usecols = [0, 1, 2], names = ['reviewerID', 'asin', 'overall']) # 2014 format
        original_df = pd.read_csv(file, usecols = [0, 1, 2], names = ['asin', 'reviewerID', 'overall']) # 2018 format
        print(original_df.dtypes)
...
def make_csv_name(full_filename: str) -> str:
    filename = full_filename.split('.')[0].split('_')
    if full_filename.split('.')[-1] == 'gz':
...
if __name__ == '__main__':
    ...
    print(f"{avg_reviewers=}")
    print(f"{avg_products=}")
